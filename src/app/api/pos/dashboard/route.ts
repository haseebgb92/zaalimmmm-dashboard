...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get top selling items for the date range (only items that were actually sold)
    const topItems = await db.execute(`
      ...
      FROM pos_products p
      INNER JOIN pos_order_items oi ON p.id = oi."productId"
      INNER JOIN pos_orders o ON oi."orderId" = o.id 
